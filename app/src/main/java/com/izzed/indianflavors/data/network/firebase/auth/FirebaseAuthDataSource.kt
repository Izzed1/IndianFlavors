...
interface FirebaseAuthDataSource { // datasource adalah sumber data
    // Buat kontrak, untuk membuat datasource yang lebih generic. Ini terjadi ketika aplikasi pindah dari layanan firebase ke layanan lain atau server sendiri
...
class FirebaseAuthDataSourceImpl(private val firebaseAuth: FirebaseAuth) : FirebaseAuthDataSource {
    ...
    override suspend fun doRegister(fullName: String, email: String, password: String): Boolean {
        val registerResult = firebaseAuth.createUserWithEmailAndPassword(email, password).await() // await(), salah satu sintax coroutine untuk menunggu result dari register, baru menyelesaikan code dibawahnya
        // coroutine :
    ...
    override fun isLoggedIn(): Boolean {
        // currentUser == user yang sedang login saat tersebut
        // currentUser == user tidak login
        return firebaseAuth.currentUser != null // verifikasi apakah user != null
    }
}